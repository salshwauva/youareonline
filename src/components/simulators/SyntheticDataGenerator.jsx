    ...
    <div className="retro-window" style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h4 style={{ color: '#222638', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.95rem' }}>
          <Sparkles size={18} color="#7c8cc6" /> Automated Synthetic Data Generator
        </h4>

        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <label style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 'bold' }}>Records:</label>
          <select 
            value={recordCount} 
            onChange={(e) => setRecordCount(Number(e.target.value))}
            style={{ background: '#ffffff', color: '#222638', border: '2px solid #222638', padding: '4px 8px', borderRadius: '4px', fontSize: '0.75rem' }}
          >
          ...
          <button onClick={generateData} className="retro-btn retro-btn-pink" style={{ padding: '4px 10px', fontSize: '0.75rem' }}>
      ...
      {!generatedData ? (
        <div style={{ padding: '24px', textAlign: 'center', background: '#ffffff', borderRadius: '4px', border: '2px solid #222638', color: '#6b7280', fontSize: '0.85rem' }}>
          Click <strong>Generate</strong> to produce synthetic JSON/CSV mock datasets for Pandas, SQL, or Data Science quests.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.75rem', color: '#7c8cc6', fontFamily: 'var(--font-code)', fontWeight: 'bold' }}>
              Output Payload ({format.toUpperCase()})
            </span>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={handleCopy} className="retro-btn" style={{ padding: '4px 8px', fontSize: '0.7rem' }}>
                <Copy size={12} /> {copied ? 'Copied!' : 'Copy'}
              </button>
              <button onClick={handleDownload} className="retro-btn retro-btn-blue" style={{ padding: '4px 8px', fontSize: '0.7rem' }}>
